...
- **Is:** reusable Cursor skills/rules for FE/BE + cross-cutting globals, adapted per project
...
| Path | Responsibility |
|------|----------------|
| `global/skills/` | Cross-cutting skills (errors, contracts, security, intake, …) |
...
3. FE/BE skills **reference** globals by name; do not paste global bodies into FE/BE skills. Edit globals only under `global/skills/`.
...
When changing a template: update README index if a skill is added/removed; keep `global` / FE / BE split clean.
